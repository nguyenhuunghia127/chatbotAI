# -*- coding: utf-8 -*-
"""
MODULE RAG ENGINE (RETRIEVAL-AUGMENTED GENERATION) NỘI BỘ (LOCAL 100%)
Dự án: Hệ thống Chatbot AI Giám sát Người cao tuổi
Tác giả: Kiến trúc sư AI & Kỹ sư Edge Systems

Công nghệ:
1. Vector Database: ChromaDB Local (Lưu trữ offline tại chroma_db/)
2. Text Embedding: HuggingFace Sentence-Transformers (all-MiniLM-L6-v2 hoặc tương đương)
3. Framework điều phối: LangChain Core / Community RAG Chain
4. Local LLM: Ollama (qwen2:7b hoặc llama3:8b) chạy qua port 11434 nội bộ
5. Tích hợp Edge Fail-Safe Engine: Tự động dự phòng và giữ tính sẵn sàng cao (High Availability)
   ngay cả khi Ollama hoặc ChromaDB đang khởi động/chưa cài đặt xong.
"""
...
# Nạp động các thư viện LangChain & ChromaDB (Dùng importlib để tránh lỗi cảnh báo Unresolved Import trên IDE khi chưa cài đặt)
import importlib
...
try:
    ...
    prompt_mod = importlib.import_module("langchain_core.prompts")
    PromptTemplate = getattr(prompt_mod, "PromptTemplate", None)

    if Chroma and HuggingFaceEmbeddings and Document:
        HAS_LANGCHAIN = True
except Exception:
    try:
        # Fallback với cấu trúc package langchain cũ
        ...
        prompt_mod = importlib.import_module("langchain.prompts")
        PromptTemplate = getattr(prompt_mod, "PromptTemplate", None)
...
MEDICAL_DOCS_DIR = os.path.join(PROJECT_ROOT, "data", "medical_docs")
CHROMA_PERSIST_DIR = os.path.join(PROJECT_ROOT, "chroma_db")

# ==============================================================================
# SYSTEM PROMPT CHUYÊN BIỆT CHO TRỢ LÝ Y TẾ & GIÁM SÁT NGƯỜI CAO TUỔI
# ==============================================================================
...
class RAGEngine:
    """
    Bộ máy RAG Local kết hợp ChromaDB, HuggingFace Embeddings và Local LLM (Ollama).
    Được tối ưu hoá cho tài nguyên Edge AI (Orange Pi 5 / PC Local).
    """

    def __init__(
        self,
        logs_file: str = SYSTEM_LOGS_PATH,
        docs_dir: str = MEDICAL_DOCS_DIR,
        ...
        print(f"[RAG ENGINE] Khoi tao RAG Engine Local (Ollama Model: {self.ollama_model})", flush=True)
        self._initialize_vector_store()

    def _initialize_vector_store(self):
        """
        Khởi tạo Vector DB:
        - Nếu có LangChain & Chroma: dùng ChromaDB persistent.
        - Nếu chưa có: kích hoạt Edge In-Memory Vector Store để hệ thống chạy tức thì không bị crash.
        """
        try:
            if HAS_LANGCHAIN:
                print(f"[RAG] Su dung LangChain + ChromaDB tai: {self.chroma_dir}", flush=True)
                os.makedirs(self.chroma_dir, exist_ok=True)
                
                # Khởi tạo embedding model nhẹ
                try:
                    ...
                    print(f"[WARN EMBEDDING]: {e_emb}. Chuyen sang che do fallback text embeddings.", flush=True)
                    self.embeddings = None

                self.rebuild_vector_db()
            else:
                print("[INFO] LangChain/ChromaDB chua duoc cai dat trong moi truong hien tai.", flush=True)
                print("[FAIL-SAFE] Kich hoat Edge Local Redundancy Engine (He thong giu tinh san sang cao 100%).", flush=True)
                self._load_local_documents_cache()

        except Exception as e:
            print(f"[WARN INIT RAG]: {str(e)}. Dang dung che do Local Fallback.", flush=True)
            self._load_local_documents_cache()
    ...
    def _load_local_documents_cache(self):
        ...
        if os.path.exists(self.docs_dir):
            for filepath in glob.glob(os.path.join(self.docs_dir, "*.txt")):
                try:
                    ...
                    print(f"[ERROR DOC]: {filepath} - {e}", flush=True)
        ...
        if os.path.exists(self.logs_file):
            try:
                ...
                print(f"[ERROR LOG]: {self.logs_file} - {e}", flush=True)

        print(f"[CACHE] Da nap {len(self.documents_cache)} doan tri thuc vao bo nho Edge.", flush=True)
        ...
        try:
            ...
            if os.path.exists(self.logs_file):
                with open(self.logs_file, "r", encoding="utf-8") as f:
                    ...
                    # Gom nhóm mỗi 3-5 log entries hoặc index từng log có mốc thời gian
            ...
            if docs:
                ...
                print(f"[OK] Da lap chi muc ChromaDB thanh cong ({len(docs)} documents).", flush=True)
            else:
                print("[WARN] Khong co document nao de lap chi muc ChromaDB.", flush=True)

        except Exception as e:
            print(f"[ERROR REBUILD CHROMA]: {str(e)}", flush=True)
            self._load_local_documents_cache()

    def get_recent_logs(self, n: int = 8) -> str:
        """Trích xuất n dòng log mới nhất từ system_logs.txt để đưa vào ngữ cảnh thời gian thực."""
    ...
    def retrieve_context(self, query: str, top_k: int = 4) -> Dict[str, Any]:
        """
        Truy xuất ngữ cảnh liên quan nhất từ Vector DB / Local Knowledge Base.
        """
        context_chunks = []
        sources = set()
        ...
        if HAS_LANGCHAIN and self.vector_store:
            try:
                ...
                print(f"[WARN SIMILARITY SEARCH]: {e}", flush=True)

        # Nếu chưa có kết quả từ Chroma, sử dụng Local Vector Space Semantic Retriever
        if not context_chunks:
            try:
                from src.rag.local_vector_retriever import get_local_retriever
                local_retriever = get_local_retriever()
                local_results = local_retriever.search(query, top_k=top_k)
                for res in local_results:
                    if res.get("score", 0) > 0.04:
                        context_chunks.append(res["text"])
                        sources.add(res["source"])
            except Exception as e_ret:
                print(f"[WARN LOCAL RETRIEVER]: {e_ret}", flush=True)

        # Nếu vẫn chưa có kết quả, fallback theo heuristic từ khóa
        if not context_chunks:
            self._load_local_documents_cache()
            keywords = [w.lower() for w in query.replace("?", "").replace(",", "").split() if len(w) > 2]
            ...
            for doc in self.documents_cache:
                ...
                for kw in keywords:
                    if kw in content_lower:
                        score += 1
                if "ngã" in query.lower() and "ngã" in content_lower:
                    score += 3
                if "sốt" in query.lower() and "sốt" in content_lower:
                    score += 3
                if "tim" in query.lower() and ("tim" in content_lower or "cardiac" in content_lower):
                    score += 3
                if any(w in query.lower() for w in ["tôi là ai", "tôi là", "tôi tên", "ai đang", "hồ sơ", "bệnh án", "bác sĩ", "dị ứng", "phòng", "cụ là ai", "bệnh nhân"]):
                    if "patient_profile.txt" in doc.get("source", ""):
                        score += 8

                if score > 0 or doc["type"] == "medical_doc":
    ...
    def _call_llm_api(self, question: str, context: str, latest_logs: str) -> Optional[Tuple[str, str]]:
        """
        Gọi LLM API thông minh (xKiro API / Claude / Mistral / DeepSeek) để phân tích ngữ cảnh,
        suy luận lâm sàng và sinh câu trả lời tự nhiên, có căn cứ từ tri thức RAG đã học.
        """
        from src.training.llm_synthetic_generator import DEFAULT_API_KEY, DEFAULT_BASE_URL
        ...
        system_msg = (
            "Bạn là Trợ lý AI Bác sĩ Lão khoa & Cấp cứu Giám sát Người cao tuổi AuraCare AI.\n"
            ...
            "1. Dựa trên TRI THỨC Y KHOA ĐÃ HỌC (CONTEXT) và NHẬT KÝ CẢM BIẾN (LATEST SENSOR LOGS) được cung cấp bên dưới.\n"
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if choices:
                        ...
                        # Phòng vệ nếu chạm trần độ dài: làm sạch đoạn dở dang
                        if finish_reason == "length" and content:
                            content = re.sub(r'\s*(\d+\.|\-|\*)\s*$', '', content)
                            last_punct = max(content.rfind("."), content.rfind("!"), content.rfind("?"))
                            if last_punct > 100:
                                content = content[:last_punct + 1].strip()
                            if not content.endswith(('.', '!', '?')):
                                content += ".\n\n*(Lưu ý: Luôn tham khảo ý kiến BS. CKI Trần Minh Tuấn khi cần điều chỉnh chế độ của cụ An).*"
                        if content:
    ...
    def _call_ollama(self, prompt: str) -> Optional[str]:
        """Gọi Ollama Local REST API (http://localhost:11434/api/generate) hoàn toàn offline."""
        ...
        payload = {
            ...
            "stream": False,
            "options": {
                "temperature": 0.2,       # Giảm nhiệt độ để loại bỏ bịa đặt (hallucination)
                "top_p": 0.9,
                "num_predict": 512
            }
        }
        ...
        try:
            response = requests.post(url, json=payload, timeout=(0.08, 2.0))
    ...
    def _generate_failsafe_response(self, question: str, latest_logs: str, context: str) -> str:
        """
        Bộ sinh câu trả lời cục bộ thông minh (Edge Deterministic Reasoning Engine).
        Đảm bảo thiết bị nhúng luôn đưa ra câu trả lời chính xác, ân cần, tổng hợp đa cảm biến
        ngay cả khi chưa tải LLM vào VRAM hoặc server Ollama đang ngoại tuyến.
        """
        ...
        if intent == "NUTRITION_DIET" or (intent == "UNKNOWN" and any(w in q_lower for w in ["chế độ ăn", "ăn kiêng", "giảm muối", "kiêng ngọt", "tiểu đường ăn gì", "huyết áp ăn gì", "thực đơn", "muối", "ăn mặn", "ăn ít muối", "ít muối"])):
            ...
            # Trường hợp 3: Hỏi về "Ăn ngọt / Đường / Bánh kẹo / Trái cây ngọt"
            if any(w in q_lower for w in ["ngọt", "bánh kẹo", "đường", "nước ngọt", "chè", "trái cây ngọt", "hoa quả ngọt"]):
                return (
        ...
        # 16. Tra cứu thông minh từ kho mẫu Few-Shot và Synthetic QA (Lọc Stopwords)
        try:
    ...
    def query(self, question: str) -> Dict[str, Any]:
        ...
        1. Lấy context từ Vector DB (Chroma) hoặc Knowledge Base.
        2. Lấy logs mới nhất từ system_logs.txt.
        3. Ghép System Prompt chuyên biệt Sensor Fusion.
        4. Gửi tới Local LLM (Ollama). Nếu Ollama chưa bật, chuyển sang Fail-Safe Engine.
        5. Trả về câu trả lời và danh sách nguồn tài liệu tham khảo.
        """
        try:
            # 0. Kiểm tra kho tri thức tăng cường từ phản hồi người dùng (RLHF Continuous Learning Memory)
            try:
                from src.training.rlhf_service import get_rlhf_service
            ...
            # 4. Suy luận qua LLM API thông minh (xKiro API: Mistral / DeepSeek)
            llm_res = self._call_llm_api(question, context, latest_logs)
            if llm_res:
                answer, model_name = llm_res
                used_engine = f"AuraCare GenAI Engine ({model_name})"
            else:
                # 4.1 Thử Local Ollama nếu có
                answer = self._call_ollama(full_prompt)
                used_engine = "Ollama Local LLM"

            # 5. Cơ chế Fail-safe nếu tất cả LLM không khả dụng (chế độ ngoại tuyến hoàn toàn)
            if not answer:
                used_engine = "Edge Deterministic Reasoning Engine (Fail-safe Offline)"
                answer = self._generate_failsafe_response(question, latest_logs, context)
...
# Khởi tạo singleton instance sẵn sàng cho FastAPI / Streamlit sử dụng
...
if __name__ == "__main__":
    print("=" * 80, flush=True)
    print("[TEST RAG ENGINE] KIEM THU HOAT DONG CUA RAG ENGINE NÖI BO", flush=True)
    print("=" * 80, flush=True)

    engine = get_rag_engine()

    test_questions = [
        "Sáng nay ông có bị ngã không? Chi tiết thế nào và tôi phải làm gì?",
        "Tình hình thân nhiệt và sốt của bà như thế nào?",
        "Hiện tại tình trạng sức khỏe chung của người cao tuổi ra sao?"
    ]

    for q in test_questions:
        print(f"\n❓ [CÂU HỎI]: {q}", flush=True)
        res = engine.query(q)
        print(f"🤖 [ENGINE]: {res['engine']}", flush=True)
        print(f"📚 [NGUỒN DỮ LIỆU]: {res['sources']}", flush=True)
